...
def pos_corpus_to_tensor(sentences, char2id, label2id, max_sent_len, max_token_len, end_of_word_token_num):
    ...
    for sent_i, sent in enumerate(sentences):
        for token_i, token in enumerate(sent):
            ...
            inputs[sent_i, token_i, 0] = end_of_word_token_num
            for char_i, char in enumerate(token.form):
                inputs[sent_i, token_i, char_i + 1] = char2id.get(char, 0)
            inputs[sent_i, token_i, char_i + 2] = end_of_word_token_num

    return inputs, targets


class POSTagger:
    def __init__(self, model, char2id, id2label, max_sent_len, max_token_len, end_of_word_token_num):
        self.model = model
        self.char2id = char2id
        self.id2label = id2label
        self.max_sent_len = max_sent_len
        self.max_token_len = max_token_len
        self.end_of_word_token_num = end_of_word_token_num
    ...
    def __call__(self, sentences):
        ...
        for sent_i, sentence in enumerate(tokenized_corpus):
            for token_i, token in enumerate(sentence):
                inputs[sent_i, token_i, 0] = self.end_of_word_token_num
                for char_i, char in enumerate(token):
                    inputs[sent_i, token_i, char_i + 1] = self.char2id.get(char, 0)
                inputs[sent_i, token_i, char_i + 2] = self.end_of_word_token_num
