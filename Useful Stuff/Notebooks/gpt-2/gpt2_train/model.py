...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        _logger.info("forcing vocab_size=50257, block_size=1024")
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        _logger.info(f"num decayed parameter tensors: {len(decay_params)}, with {num_decay_params:,} parameters")
        _logger.info(f"num non-decayed parameter tensors: {len(nodecay_params)}, with {num_nodecay_params:,} parameters")
        # create AdamW optimizer and use the fused version if it is available
        fused_available = 'fused' in inspect.signature(torch.optim.AdamW).parameters
        use_fused = fused_available and 'cuda' in device
        _logger.info(f"using fused AdamW: {use_fused}")
